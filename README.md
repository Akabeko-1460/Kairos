...
設計・仕様・技術選定の詳細は [`docs/`](./docs) を参照してください。特に `CLAUDE.md`（リポジトリ直下）が作業指示の起点です。
...
- `docs/research/` — サウンド設計の根拠にした外部文献調査（一覧は `docs/README.md` 参照）
...
docs/                     設計ドキュメント・文献調査
assets/                   リポジトリ直下で管理するブランディング素材（アプリには未組み込み）
```
