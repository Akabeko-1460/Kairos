...
下表の「表示すべきクレジット」列の内容を掲載すること（**掲載済み**。
`apps/web/app/credit/page.tsx` の `SOUND_CREDITS`。素材を追加したら同配列も更新すること）。
...
- [x] CC BY / CC BY-SA 素材のクレジットを `apps/web/app/credit` ページに掲載する

## 実音源の保護（重要）

上表の実音源（Cell 19件 / Relax の Texture 2件 / Cue 2件 = 計23件）の**加工元の生ファイルは
リポジトリに含まれていない**（`process-real-audio.mjs` の入力は当時の一時ディレクトリ）。
そのため、これらを上書きすると出所URLから6本を再取得しない限り復元できない。

`scripts/generate-placeholder-audio.mjs` はこの23件を既定でスキップする
（同ファイルの `REAL_AUDIO_OWNED`）。合成音のプレースホルダーへ意図的に戻したい場合のみ
`KAIROS_REGENERATE_ALL=1` を指定すること。素材を実音源へ差し替えたら、
`REAL_AUDIO_OWNED` にもそのパスを追加する。
