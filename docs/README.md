...
`CLAUDE.md` → `03_ARCHITECTURE.md` の ADR → `04_SOUND_ENGINE.md` の順に読んでください。
...
> **番号付きの `0X_*.md` は着手前に書いた初期の設計資料です。**
> 実装が先へ進んだ結果、記述がずれている箇所があります。
> **実装と食い違ったら常に実装（コードと `packs.json`）が正。**
> ただし `03_ARCHITECTURE.md` の ADR-001〜011 だけは例外で、現役の設計判断の記録として
> ソースコメントから約50箇所参照されています（`ADR-009` 等の番号で検索可能）。

| ファイル | 内容 | 位置づけ |
|---|---|---|
| `CLAUDE.md` | Claude Code 向けの作業指示・規約・禁止事項（最初に読む） | **現役** |
| `03_ARCHITECTURE.md` | **ADR-001〜011**（設計判断の記録）+ 技術選定・ディレクトリ構成 | **ADR は現役**／それ以外は要注意 |
| `04_SOUND_ENGINE.md` | 生成BGMエンジンの設計意図。テーマ別の音作りの根拠 | 設計意図は現役／**数値の正は `packs.json`** |
| `ASSET_LICENSES.md` | 音素材・IRのライセンス台帳と実音源の保護方針 | **現役**（素材追加時は必ず更新） |
| `research/` | サウンド設計の根拠にした外部文献調査（下記） | **現役**（ADR から参照） |
| `02_SPEC.md` | 機能要件（F-01〜F-29）・画面仕様・データモデル | 要件IDは現役／画面仕様は初期案（Timer/Stopwatch 追加前） |
| `01_ENDEL_RESEARCH.md` | Endel の調査結果。何を真似て何を真似ないか | 初期の背景資料。コードからの参照なし |
| `05_IMPLEMENTATION_PLAN.md` | Phase 0〜4 のタスク分解と受け入れ条件 | 初期の計画。進捗の正は `CLAUDE.md` のチェックリスト |
| `PHASE0_SPIKES.md` | Phase 0 の3スパイクの検証結果 | 完了済みの記録 |
